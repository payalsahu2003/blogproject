...
const Auth = () => {
  ...
  const sendRequest = async (type = "login") => {
    const res = await axios.post(`http://localhost:6000/api/user/${type}`, {
      name: inputs.name,
      email: inputs.email,
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <Box
          maxWidth={400}
          display={"flex"} flexDirection={"column"} alignItems="center" justifyContent={"center"} boxShadow="10px 10px 20px #ccc" padding={3}    margin="auto" marginTop={5} borderRadius={5}
        >
